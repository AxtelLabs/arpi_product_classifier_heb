...
class App(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
       ...
       # self.attributes("-toolwindow",1)
        ...
        #self.geometry("{0}x{1}+0+0".format(1280, 800))
...
class PageOne(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # Add canvas to frame
        self.can = tk.Canvas(self, width=WIDTH, height=HEIGHT, bg='#ffffff')
        ...
        self.vidCont.place(x=int(WIDTH/12), y=int(HEIGHT/8), width=int(640) , height=int(480))
        ...
        self.w_img3, self.h_img3 = self.img3.size 
        self.img3= self.img3.resize((int(self.w_img3/6),int(self.h_img3/6)), PIL.Image.ANTIALIAS)
        self.img3= PIL.ImageTk.PhotoImage(self.img3)

        # HEB logo
        self.logoRaw= PIL.Image.open(path + "imgs/logo.png")
        self.wheb, self.hheb =self.logoRaw.size
        self.logoRaw= self.logoRaw.resize((int(self.wheb/20),int(self.hheb/20)), PIL.Image.ANTIALIAS)
        ...
        self.w1anim, self.h1anim =self.anim1Raw.size
        self.anim1Raw= self.anim1Raw.resize((int(self.w1anim/2),int(self.h1anim/2)), PIL.Image.ANTIALIAS)
        ...
        self.w1, self.h1 =self.inst1Raw.size
        self.inst1Raw= self.inst1Raw.resize((int(self.w1/3),int(self.h1/3)), PIL.Image.ANTIALIAS)
        ...
        self.w2, self.h2 =self.inst2Raw.size
        self.inst2Raw= self.inst2Raw.resize((int(self.w2/3),int(self.h2/3)), PIL.Image.ANTIALIAS)
        ...
        self.w3, self.h3 =self.inst3Raw.size
        self.inst3Raw= self.inst3Raw.resize((int(self.w3/10),int(self.h3/10)), PIL.Image.ANTIALIAS)
        ...
        self.w4, self.h4 =self.inst4Raw.size
        self.inst4Raw= self.inst4Raw.resize((int(self.w4/10),int(self.h4/10)), PIL.Image.ANTIALIAS)
        self.inst4= PIL.ImageTk.PhotoImage(self.inst4Raw)

        # Instructions 5
        self.inst5Raw= PIL.Image.open(path + "imgs/inst5.png")
        self.w5, self.h5 =self.inst5Raw.size
        self.inst5Raw= self.inst4Raw.resize((int(self.w5/10),int(self.h5/10)), PIL.Image.ANTIALIAS)
        self.inst5= PIL.ImageTk.PhotoImage(self.inst5Raw)
        self.lastMsg = self.can2.create_image(int(HEIGHT/2),int(WIDTH/2), image=self.inst5)
...
class PageTwo(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.wuno, self.huno =self.uno.size
        self.uno= self.uno.resize((int(self.wuno/4),int(self.huno/4)), PIL.Image.ANTIALIAS)
        ...
        self.wanim1, self.hanim1 =self.anim1Raw.size
        self.anim1Raw= self.anim1Raw.resize((int(self.wanim1/2),int(self.hanim1/2)), PIL.Image.ANTIALIAS)
        self.anim1= PIL.ImageTk.PhotoImage(self.anim1Raw)
        self.label3= tk.Label(self, background= "#FFFFFF", image= self.anim1) 
        ...
        self.wdos, self.hdos =self.dos.size
        self.dos= self.dos.resize((int(self.wdos/6),int(self.hdos/6)), PIL.Image.ANTIALIAS)
        ...
        self.wdos, self.hdos =self.dos.size
        self.dos= self.dos.resize((int(self.wdos/4),int(self.hdos/4)), PIL.Image.ANTIALIAS)
        self.dosre=PIL.ImageTk.PhotoImage(self.dos)
        self.lastMsg = self.can2.create_image(int(HEIGHT*0.75),int(WIDTH/4), image=self.dosre)

        # Finish button
        self.finish= PIL.Image.open(path + "imgs/terminar.png")
        self.wff, self.hff =self.finish.size
        self.finish= self.finish.resize((int(self.wff/1.5),int(self.hff/1.5)), PIL.Image.ANTIALIAS)
        ...
        self.ws, self.hs =self.search.size
        self.search= self.search.resize((int(self.ws/3),int(self.hs/3)), PIL.Image.ANTIALIAS)
...
class PageThree(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # Keyboard body - ROW 1
        q = tk.Button(self.can3,text = 'Q' ,font=("Helvetica","18"), width = int(5), height = int(3), command = lambda : self.press('Q'))
        q.grid(row = 1 , column = 0, ipadx = xPadVal , ipady = yPadVal)

        w = tk.Button(self.can3,text = 'W' , font=("Helvetica","18"), width = int(5), height = int(3), command = lambda : self.press('W'))
        w.grid(row = 1 , column = 1, ipadx = xPadVal , ipady = yPadVal)

        E = tk.Button(self.can3,text = 'E' ,font=("Helvetica","18"), width = int(5), height = int(3), command = lambda : self.press('E'))
        E.grid(row = 1 , column = 2, ipadx = xPadVal , ipady = yPadVal)

        R = tk.Button(self.can3,text = 'R' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('R'))
        R.grid(row = 1 , column = 3, ipadx = xPadVal , ipady = yPadVal)

        T = tk.Button(self.can3,text = 'T' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('T'))
        T.grid(row = 1 , column = 4, ipadx = xPadVal , ipady = yPadVal)

        Y = tk.Button(self.can3,text = 'Y' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('Y'))
        Y.grid(row = 1 , column = 5, ipadx = xPadVal , ipady = yPadVal)

        U = tk.Button(self.can3,text = 'U' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('U'))
        U.grid(row = 1 , column = 6, ipadx = xPadVal , ipady = yPadVal)

        I = tk.Button(self.can3,text = 'I' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('I'))
        I.grid(row = 1 , column = 7, ipadx = xPadVal , ipady = yPadVal)

        O = tk.Button(self.can3,text = 'O' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('O'))
        O.grid(row = 1 , column = 8, ipadx = xPadVal , ipady = yPadVal)

        P = tk.Button(self.can3,text = 'P' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('P'))
        P.grid(row = 1 , column = 9, ipadx = xPadVal , ipady = yPadVal)


        # Keybaord body - ROW 2
        A = tk.Button(self.can3,text = 'A' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('A'))
        A.grid(row = 2 , column = 0, ipadx = xPadVal , ipady = yPadVal)
        

        S = tk.Button(self.can3,text = 'S' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('S'))
        S.grid(row = 2 , column = 1, ipadx = xPadVal , ipady = yPadVal)

        D = tk.Button(self.can3,text = 'D' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('D'))
        D.grid(row = 2 , column = 2, ipadx = xPadVal , ipady = yPadVal)

        F = tk.Button(self.can3,text = 'F' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('F'))
        F.grid(row = 2 , column = 3, ipadx = xPadVal , ipady = yPadVal)

        G = tk.Button(self.can3,text = 'G' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('G'))
        G.grid(row = 2 , column = 4, ipadx = xPadVal , ipady = yPadVal)

        H = tk.Button(self.can3,text = 'H' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('H'))
        H.grid(row = 2 , column = 5, ipadx = xPadVal , ipady = yPadVal)

        J = tk.Button(self.can3,text = 'J' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('J'))
        J.grid(row = 2 , column = 6, ipadx = xPadVal , ipady = yPadVal)

        K = tk.Button(self.can3,text = 'K' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('K'))
        K.grid(row = 2 , column = 7, ipadx = xPadVal , ipady = yPadVal)

        L = tk.Button(self.can3,text = 'L' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('L'))
        L.grid(row = 2 , column = 8, ipadx = xPadVal , ipady = yPadVal)

        Ñ = tk.Button(self.can3,text = 'Ñ' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('Ñ'))
        ...
        # Keybaord body - ROW 3
        Z = tk.Button(self.can3,text = 'Z' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('Z'))
        Z.grid(row = 3 , column = 0, ipadx = xPadVal , ipady = yPadVal)


        X = tk.Button(self.can3,text = 'X' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('X'))
        X.grid(row = 3 , column = 1, ipadx = xPadVal , ipady = yPadVal)


        C = tk.Button(self.can3,text = 'C' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('C'))
        C.grid(row = 3 , column = 2, ipadx = xPadVal , ipady = yPadVal)


        V = tk.Button(self.can3,text = 'V' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('V'))
        V.grid(row = 3 , column = 3, ipadx = xPadVal , ipady = yPadVal)

        B = tk.Button(self.can3, text= 'B' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('B'))
        B.grid(row = 3 , column = 4 , ipadx = xPadVal , ipady = yPadVal)


        N = tk.Button(self.can3,text = 'N' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('N'))
        N.grid(row = 3 , column = 5, ipadx = xPadVal , ipady = yPadVal)


        M = tk.Button(self.can3,text = 'M' , font=("Helvetica","18"),width = int(5), height = int(3), command = lambda : self.press('M'))
        M.grid(row = 3 , column = 6, ipadx = xPadVal , ipady = yPadVal)

        clear = tk.Button(self.can3,text = '<- Retroceso' ,font=("Helvetica","16"), width = int(100), height = int(3), command = lambda : self.clear())
        clear.place(relx=0.78, rely=0.01,relwidth=0.2, relheight=.15)

        enter = tk.Button(self.can3,text = "ENTER" ,font=("Helvetica","16"), width = int(70), height = int(3), command = lambda : self.action())
        ...
        # Finish button
        self.finish= PIL.Image.open(path + "imgs/terminar.png")
        self.wf, self.hf =self.finish.size
        self.finish= self.finish.resize((int(self.wf/3),int(self.hf/3)), PIL.Image.ANTIALIAS)
        self.finishre=PIL.ImageTk.PhotoImage(self.finish)

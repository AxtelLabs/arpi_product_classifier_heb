...
class Combobox_Autocomplete(Entry, object):
    def __init__(self, master, list_of_items, string,autocomplete_function=None,listbox_width=680, listbox_height=5, ignorecase_match=False, startswith_match=False, vscrollbar=True, hscrollbar=False, **kwargs):
    ...
    def _build_listbox(self, values):
        ...
        self._listbox = Listbox(listbox_frame, background="#e6e6e6", fg="#666666", selectmode=SINGLE, activestyle="none", exportselection=False, font=("Helvetica", 20), height=35)
        ...
        if self._listbox is not None:
           ...
           # print(current_selection)#prints listbox index of the selection
            if current_selection:
                ...
                #text = text.replace(" ", "      ")
                #productAndPlu = Label(self.master,text=text)
                #productAndPlu.place(x= 500, y= 400)
                #productAndPlu.focus()
                self._set_var(text)
